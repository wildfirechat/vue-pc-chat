...
class WfcImpl {
    ...
    async sendMessageEx(message, toUsers = [], preparedCB, progressCB, successCB, failCB) {
        ...
        if (message.messageContent instanceof MediaMessageContent && this.isSupportBigFilesUpload()) {
    ...
    _uploadXMLHttpRequest(message, fileName, remoteUrl, progressCB, successCB, failCB) {
    ...
    _uploadFile(message, file, mediaType, progressCB, successCB, failCB) {
        this.getUploadMediaUrl(file.name, mediaType, `application/octet-stream`, (uploadUrl, remoteUrl, backUploadUrl, serverType) => {
            let xhr;
            if (serverType === 0) {
                // 内置存储，不支持大文件上传
                ...
                let token = ss[1];
                let key = ss[2];
                xhr = this._uploadXMLHttpRequest(message, file.name, remoteUrl, progressCB, successCB, failCB);
