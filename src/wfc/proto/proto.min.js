...
import { gt, numberValue, stringValue } from '../util/longUtil';
...
class WfcImpl {
    ...
    onConnectionChanged(status) {
        ...
        if (status === ConnectionStatus.ConnectionStatusConnected) {
            ...
            if (Config.getOpenPlatformWorkSpaceUrl()) {
    ...
    onErrorEvent(errType, errMsg) {
        ...
        if(errMsg === 1 && errMsg.indexOf('im.wildfirechat.cn') >= 0) {
            console.error('请修改 im-server 配置文件里面的 server.ip。修改后，需要重新 im-server!!!');
        }
    }

    onJoinGroupRequestUpdate() {
        self.eventEmitter.emit(EventType.JoinGroupRequestUpdate);
    }
    ...
    init(args = []) {
        ...
        if(process.platform === 'openharmony'){
            // 临时方案
            protoProxy.invoke('setDBPath', "/data/storage/el2/base/files")
            //protoProxy.invoke('setPackageName', "cn.wildfirechat.chat.pc")
            // protoProxy.invoke('setDeviceInfo', 12, "pc", "1.0", "mateBook pro");
        }
        protoProxy.invoke('setLanguage', Config.LANGUAGE);

        //KCP需要专业版IM服务支持，并打开KCP端口。这个函数要在onCreate之前调用，否则会无效.
        //this.setUseKcp(88, true);

        this._setupEventListeners();
        this._syncConnectedToMainNetwork();
    ...
    // 子窗口 attach，或者渲染进程 crash 后重新加载时，收不到之前的 connectedToServer 事件，需要主动同步当前连接的网络
    _syncConnectedToMainNetwork() {
        // 1 主网络；-1 备选网络；0 未知
        this._connectedToMainNetwork = protoProxy.invoke('getConnectedNetworkType') !== -1;
    }
    ...
    setDataVerify(enabled) {
        protoProxy.invoke('useDataVerify', enabled);
    }

    setEncryptMaxMinutes(minutes) {
        protoProxy.invoke('useEncryptMaxMinutes', minutes);
    }
    ...
    setUseKcp(kcpPort, useKcp) {
        protoProxy.invoke('setUseKcp', kcpPort, useKcp);
    }

    isUseKcp() {
        return protoProxy.invoke('isUseKcp');
    }

    setUseWebsocket(enable) {
        protoProxy.invoke('setUseWebsocket', enable);
    }

    isUseWebsocket() {
        return protoProxy.invoke('isUseWebsocket');
    }

    useTls(skipVerifyCert, selfSignedCerts) {
        protoProxy.invoke('useTls', skipVerifyCert, selfSignedCerts);
    }
    ...
    connect(userId, token) {
        ...
        if (!self.userId) {
            ...
            if (status !== ConnectionStatus.ConnectionStatusConnecting
    ...
    setDeviceName(deviceName) {
        protoProxy.invoke('setDeviceName', deviceName);
    }
    ...
    resetClientId() {
        protoProxy.invoke('resetClientId');
    }
    ...
    getUserInfosAsync(userIds, groupId = '', callback) {
        ...
        protoProxy.invokeAsync('getUserInfos', userIds, groupId, userInfoStrs => {
            let users = [];
            if (userInfoStrs && userInfoStrs !== '') {
                let tmp = JSON.parse(userInfoStrs);
                tmp.forEach((u) => {
    ...
    getGroupMembersEx2(groupId, offset, count, types = []) {
        let membersStr = protoProxy.invoke('getGroupMembersEx2', groupId, types, offset, count);
        var members = [];
        let arr = JSON.parse(membersStr);
        arr.forEach(e => {
            members.push(Object.assign(new GroupMember(), e));
        });
        return members;
    ...
    getGroupMembersCount(groupId, types = []) {
        return protoProxy.invoke('getGroupMembersCount', groupId, types);
    }

    getGroupMemberIdsEx(groupId, types = []) {
      let result = protoProxy.invoke('getGroupMemberIds', groupId, types);
      return JSON.parse(result);
    }

    loadGroupMemberFromRemote(groupId) {
        protoProxy.invoke('loadGroupMemberFromRemote', groupId);
    }
    ...
    sendJoinGroupRequest(groupId, memberIds, reason, extra, successCB, failCB) {
        protoProxy.invokeAsync('sendJoinGroupRequest', groupId, memberIds, reason, extra, () => {
            if (successCB) {
                successCB();
            }
        }, (errorCode) => {
    ...
    handleJoinGroupRequest(groupId, memberId, inviter, status, memberExtra, successCB, failCB) {
        protoProxy.invokeAsync('sendJoinGroupRequest', groupId, memberId, inviter, status, memberExtra, () => {
            if (successCB) {
                successCB();
            }
        }, (errorCode) => {
    ...
    getJoinGroupRequests(groupId, memberId, status) {
        let result = protoProxy.invoke('getJoinGroupRequests', groupId, memberId, status);

        let joinGroupRequestListStr = JSON.parse(result);
        let joinGroupRequestList = [];
        if (joinGroupRequestListStr && joinGroupRequestListStr.length > 0) {
            joinGroupRequestListStr.forEach((r) => {
                joinGroupRequestList.push(Object.assign(new JoinGroupRequest(), r));
            });
        }
        return joinGroupRequestList;
    }

    clearJoinGroupRequest(groupId, memberId, inviter) {
        return protoProxy.invoke('clearJoinGroupRequest', groupId, memberId, inviter);
    }

    getAllJoinGroupRequestUnread() {
        return protoProxy.invoke('getAllJoinGroupRequestUnread');
    }

    getJoinGroupRequestUnread(groupId) {
        return protoProxy.invoke('getJoinGroupRequestUnread', groupId);
    }

    clearJoinGroupRequestUnread(groupId) {
        protoProxy.invoke('clearJoinGroupRequestUnread', groupId);
    }

    clearRemoteJoinGroupRequest(successCB, failCB) {
        protoProxy.invokeAsync('clearRemoteJoinGroupRequest', () => {
            if (successCB) {
                successCB();
            }
        }, (errorCode) => {
    ...
    getUserSettingsLike(scope, keyPrefix) {
        let result = protoProxy.invoke('getUserSettingsLike', scope, keyPrefix);
        return JSON.parse(result);
    }
    ...
    searchMessageByTypesAsync(conversation, keyword, contentTypes, desc, limit, offset, withUser = '', resultCB = null) {
        ...
        protoProxy.invokeAsync('searchMessageByTypes', JSON.stringify(conversation), keyword, contentTypes, desc, limit, offset, withUser, result => {
            let msgs = JSON.parse(result);
            let matchMsgs = [];
            if (msgs && msgs.length > 0) {
                msgs.forEach(m => {
        ...
        if (message.messageContent instanceof MediaMessageContent && (message.messageContent.localPath || message.messageContent.file) && !message.messageContent.remotePath && this.isSupportBigFilesUpload()) {
            ...
            if ((process.platform === 'darwin' || process.platform === "win32") && message.messageContent instanceof ImageMessageContent) {
                ...
                if (message.messageContent.localPath) {
                    ...
                    // electronv24版本后，在渲染线程获取图片缩略图无法正确resolve
                    const thumbnail = await ipcRenderer.invoke('create-thumbnail',
            ...
            this._uploadFile(insertedMsg, file, message.messageContent.mediaType, (uploaded, total) => {
                ...
                // why?
                // vue3 reactive 机制，这儿修改的话，没有经过 proxy，直接修改了，未触发响应式
                // 后续 store.js 里面通过 proxy 去修改时，值相同，继续不能触发响应式
                let clonedMsg = Object.assign({}, insertedMsg);
                const scb =  (messageUid, timestamp) => {
                    messageUid = Long.fromValue(messageUid);
                    clonedMsg.messageUid = messageUid;
                    clonedMsg.timestamp = timestamp;
                    clonedMsg.status = MessageStatus.Sent;
                    console.log('send saved msg success', messageUid, clonedMsg);
                    if (clonedMsg.conversation.type !== ConversationType.ChatRoom) {
                        this.eventEmitter.emit(EventType.MessageStatusUpdate, clonedMsg);
                    }
                    successCB && successCB(clonedMsg.messageUid, clonedMsg.timestamp);
                }
                const ecb =  err => {
                    clonedMsg.status = MessageStatus.SendFailure;
                ...
                if (insertedMsg.messageId > 0) {
                    protoProxy.invokeAsync('sendSavedMessage', insertedMsg.messageId, 0, scb, ecb)
                } else {
                    let content = insertedMsg.messageContent.encode()
                    let strCont = JSON.stringify(content);
                    protoProxy.invokeAsync('sendSavedMessageEx', insertedMsg.messageId, strConv, strCont, 0, [], (messageId, tiemstamp) =>{}, (progress, total)=> {}, scb, ecb)
                }
    ...
    getMyCustomState() {
        this.getUserSetting(UserSettingScope.CustomState, '');
    }
    ...
    getMessageCountByDay(conversation, contentTypes, startTime, endTime) {
        ...
        let countStr = protoProxy.invoke('getMessageCountByDay', JSON.stringify(conversation), contentTypes, startTime, endTime);
    ...
    _setupEventListeners() {
        ...
        protoProxy.setProtoEventListener('joinGroupRequestUpdate', () => {
            self.onJoinGroupRequestUpdate();
        });
    }
