...
import protoRenderer from "./proto.renderer";
...
class WfcImpl {
    ...
    // refactor
    _forwardToOtherWindow(event, ...args) {
        let windows = BrowserWindow.getAllWindows();
        if (windows.length > 1) {
            ipcRenderer.send('wfcEvent', event, ...args)
        }
    ...
    init(args = []) {
        ...
        proto.setConnectToServerListener((host, ip, port) => {
            ...
            this._forwardToOtherWindow('connectToServer', host, ip, port);
        });
        ...
        proto.setReceiveMessageListener((messages, hasMore) => {
            ...
            this._forwardToOtherWindow('onReceiveMessage', messages, hasMore)

        }, (operatorUid, messageUid) => {
            // onRecallMessage
            self.onRecallMessage(operatorUid, messageUid);
            this._forwardToOtherWindow('onRecallMessage', operatorUid, messageUid)

        }, (messageUid) => {
            // onDeleteRemoteMessage
            self.onDeleteRemoteMessage(messageUid);
            this._forwardToOtherWindow('onDeleteRemoteMessage', messageUid)

        }, (receivedMapStr) => {
            // onUserReceivedMessage
            self.onUserReceivedMessage(receivedMapStr)
            this._forwardToOtherWindow('onUserReceivedMessage', receivedMapStr)

        }, (readedMapStr) => {
            // onUserReadedMessage
            self.onUserReadedMessage(readedMapStr)
            this._forwardToOtherWindow('onUserReadedMessage', readedMapStr)
        ...
        proto.setUserInfoUpdateListener((userIds) => {
            ...
            this._forwardToOtherWindow('userInfoUpdate', userIds)
        });
        ...
        proto.setFriendUpdateListener((friendListIds) => {
            ...
            this._forwardToOtherWindow('friendUpdate', friendListIds)
        });
        ...
        proto.setFriendRequestListener((newRequests = '[]') => {
            ...
            this._forwardToOtherWindow('friendRequestUpdate', newRequests)
        });
        ...
        proto.setChannelInfoUpdateListener((channelListIds) => {
            ...
            this._forwardToOtherWindow('channelInfoUpdate', channelListIds)
        });
        ...
        proto.setGroupMemberUpdateListener((groupId, groupMembersStr) => {
            ...
            this._forwardToOtherWindow('groupMemberUpdate', groupId, groupMembersStr)
        });

        proto.setSecretChatStateListener((targetId) => {
            self.onSecretChatStateChangedListener(targetId);
            this._forwardToOtherWindow('secretChatStateChange', targetId)
        });
        proto.setSecretMessageBurnStateListener((targetId, playedMessageId) => {
            self.onSecretMessageStartBurningListener(targetId, playedMessageId);
            this._forwardToOtherWindow('secretMessageStartBurn', targetId, playedMessageId)
        }, (messageIds) => {
            self.onSecretMessageBurnedListener(messageIds);
            this._forwardToOtherWindow('secretMessageBurned', messageIds)
        });
    ...
    attach(para) {
        proto = para;
        this.connectionStatus = proto.getConnectionStatus();
        if (this.connectionStatus === ConnectionStatus.ConnectionStatusConnected) {
            this.isLogined = true;
            // fixme
            this.userId = proto.getCurrentUserId();
        }
        let listenersMap = {
            'connectionStatus': self.onConnectionChanged,
            'connectToServer': self.onConnectToServer,
            'onReceiveMessage': self.onReceiveMessage,
            'onRecallMessage': self.onRecallMessage,
            'onDeleteRemoteMessage': self.onDeleteRemoteMessage,
            'onUserReceivedMessage': self.onUserReceivedMessage,
            'onUserReadedMessage': self.onUserReadedMessage,
            'conferenceEvent': self.onConferenceEvent,
            'onlineEvent': self.onOnlineEvent,
            'userInfoUpdate': self.onUserInfoUpdate,
            'friendUpdate': self.onFriendListUpdate,
            'friendRequestUpdate': self.onFriendRequestUpdate,
            'groupInfoUpdate': self.onGroupInfoUpdate,
            'settingUpdate': self.onSettingUpdate,
            'channelInfoUpdate': self.onChannelInfoUpdate,
            'groupMemberUpdate': self.onGroupMemberUpdateListener,
        };

        ipcRenderer.on('wfcEvent', (event, wfcEvent, args) => {
            listenersMap[wfcEvent](...args);
        })
        self.registerDefaultMessageContents();
    }
    ...
    getOneFriendRequest(userId, incoming) {
        let result = proto.getFriendRequest(userId, incoming);
    ...
    loadRemoteMessages(conversation, contentTypes, beforeUid, count, successCB, failCB) {
        ...
        proto.getRemoteMessages(JSON.stringify(conversation), stringValue(beforeUid), count, (protoMsgsStr) => {
    ...
    loadRemoteMessage(messageUid, successCB, failCB) {
        proto.getRemoteMessage(stringValue(messageUid), (protoMsgsStr) => {
            var protoMsgs = JSON.parse(protoMsgsStr);
            let msgs = [];
            protoMsgs.map(m => {
                let msg = Message.fromProtoMessage(m);
    ...
    async setGroupRemark(groupId, remark, successCB, failCB) {
        proto.setGroupRemark(groupId, remark, () => {
            successCB && successCB();
        }, (errorCode) => {
            failCB && failCB(errorCode);
        });
    ...
    getGroupRemark(groupId) {
        return proto.setGroupRemark(groupId);
    }
    ...
    modifyMyInfo(modifyMyInfoEntries, successCB, failCB) {
        proto.modifyMyInfo(modifyMyInfoEntries[0].type, modifyMyInfoEntries[0].value, () => {
            if (successCB) {
                successCB();
            }
        }, (errorCode) => {
    ...
    getRemoteListenedChannels(successCB, failCB) {
        proto.getRemoteListenedChannels((channelIds) => {
    ...
    clearMessageUnreadStatus(messageId) {
        proto.clearMessageUnreadStatus(messageId);
    }
    ...
    getMessagesEx2(conversationTypes, lines, status, fromIndex, before = true, count = 20, withUser = '') {
        let protoMsgsStr = proto.getMessagesEx2(conversationTypes, lines, status, fromIndex, before, count, withUser);
        // let protoMsgsStr = proto.getMessages('xxx', [0], fromIndex, before, count, withUser);
        var protoMsgs = JSON.parse(protoMsgsStr);
        let msgs = [];
        protoMsgs.map(m => {
    ...
    async sendMessageEx(message, toUsers = [], preparedCB, progressCB, successCB, failCB) {
        ...
        if (message.messageContent instanceof MediaMessageContent && !message.messageContent.remotePath && this.isSupportBigFilesUpload()) {
            ...
            this._uploadFile(insertedMsg, message.messageContent.file, message.messageContent.mediaType, (uploaded, total) => {
                ...
                protoRenderer.invokeAsync('sendSavedMessage', [insertedMsg.messageId, 0], (messageUid, timestamp) => {
        ...
        let msgStr = proto.sendMessage(strConv, strCont, toUsers, 0,
    ...
    async sendSavedMessage(message, expireDuration, successCB, failCB) {
        protoRenderer.invokeAsync('sendSavedMessage', [message.messageId, expireDuration],
            (messageUid, timestamp) => { //successCB
                message.status = MessageStatus.Sent;
                message.messageUid = Long.fromValue(messageUid);
                message.timestamp = Long.fromValue(timestamp).toNumber();
    ...
    cancelSendingMessage(messageId) {
        let canceled = proto.cancelSendingMessage(messageId);
        if (!canceled) {
    ...
    async _uploadFile(message, file, mediaType, progressCB, successCB, failCB) {
        ...
        if (message.conversation.type === ConversationType.SecretChat) {
            ...
            let encodedAb = proto.encodeSecretChatMediaData(message.conversation.target, arrayBuffer);
    ...
    getUploadMediaUrl(fileName, mediaType, contentType, successCB, failCB) {
        protoRenderer.invokeAsync('getUploadMediaUrl', [fileName, mediaType, contentType], successCB, failCB);
    }
    ...
    clearAllMessages(removeConversation) {
        proto.clearAllMessages(removeConversation);
    }
    ...
    insertMessage(conversation, messageContent, status, notify = false, serverTime = 0) {
        let protoMessageContent = messageContent.encode();
        let mStr = protoRenderer.invoke('insertMessage', JSON.stringify(conversation), self.userId, JSON.stringify(protoMessageContent), status, notify, serverTime);
        return Message.fromProtoMessage(JSON.parse(mStr));
    }

    async updateMessageContent(messageId, messageContent) {
        let protoMessageContent = messageContent.encode();
        protoRenderer.invoke('updateMessage', messageId, JSON.stringify(protoMessageContent));
    }

    async updateMessageStatus(messageId, status) {
        protoRenderer.invoke('updateMessageStatus', messageId, status);
    }
    ...
    async destroySecretChat(targetId, successCB, failCB) {
        proto.destroySecretChat(targetId,
            () => {
                if (successCB) {
                    successCB();
                }
    ...
    getSecretChatInfo(targetId) {
        let secretChatInfoStr = proto.getSecretChatInfo(targetId);
    ...
    decodeSecretChatMediaData(targetId, mediaDatas) {
        return proto.decodeSecretChatMediaData(targetId, mediaDatas);
    }

    //byte[]
    encodeSecretChatMediaData(targetId, mediaDatas) {
        return proto.encodeSecretChatMediaData(targetId, mediaDatas);
    }

    setSecretChatBurnTime(targetId, ms) {
        proto.setSecretChatBurnTime(targetId, ms);
    }

    isEnableSecretChat() {
        return proto.isEnableSecretChat();
    }

    isUserEnableSecretChat() {
        return proto.isUserEnableSecretChat();
    }

    async setUserEnableSecretChat(enable, successCB, failCB) {
        proto.setUserEnableSecretChat(enable, () => {
            successCB && successCB();
        }, (errorCode) => {
            failCB && failCB(errorCode);
        });
