...
class WfcImpl {
    ...
    getMessagesV2(conversation, fromIndex, before, count, withUser, contentTypes, successCB, failCB) {
        let protoMsgsStr = protoProxy.invoke('getMessagesV2', JSON.stringify(conversation), contentTypes, fromIndex, before, count, withUser, (protoMsgsStr) => {
            let protoMsgs = JSON.parse(protoMsgsStr);
            let msgs = [];
            protoMsgs.map(m => {
                let msg = Message.fromProtoMessage(m);
    ...
    getMessagesExV2(conversationTypes, lines, contentTypes, fromIndex, before, count, withUser, successCB, failCB) {
        let protoMsgsStr = protoProxy.invoke('getMessagesExV2', conversationTypes, lines, contentTypes, fromIndex, before, count, withUser, (protoMsgsStr) => {
            let protoMsgs = JSON.parse(protoMsgsStr);
            let msgs = [];
            protoMsgs.map(m => {
                let msg = Message.fromProtoMessage(m);
    ...
    getMessagesEx2V2(conversationTypes, lines, status, fromIndex, before, count, withUser, successCB, failCB) {
        let protoMsgsStr = protoProxy.invoke('getMessagesEx2V2', conversationTypes, lines, status, fromIndex, before, count, withUser, (protoMsgsStr) => {
            let protoMsgs = JSON.parse(protoMsgsStr);
            let msgs = [];
            protoMsgs.map(m => {
                let msg = Message.fromProtoMessage(m);
    ...
    getMessagesByTimestampV2(conversation, contentTypes, timestamp, before, count, withUser, successCB, failCB) {
        let protoMsgsStr = protoProxy.invoke('getMessagesByTimestampV2', JSON.stringify(conversation), contentTypes, timestamp, before, count, withUser, (protoMsgsStr) => {
            let protoMsgs = JSON.parse(protoMsgsStr);
            let msgs = [];
            protoMsgs.map(m => {
                let msg = Message.fromProtoMessage(m);
    ...
    getUserMessagesV2(userId, conversation, fromIndex, before, count, contentTypes, successCB, failCB) {
        let protoMsgsStr = protoProxy.invoke('getUserMessagesV2', userId, JSON.stringify(conversation), contentTypes, fromIndex, before, count, (protoMsgsStr) => {
            let protoMsgs = JSON.parse(protoMsgsStr);
            let msgs = [];
            protoMsgs.map(m => {
                let msg = Message.fromProtoMessage(m);
    ...
    getUserMessagesExV2(userId, conversationTypes, lines, fromIndex, before, count, contentTypes, successCB, failCB) {
        let protoMsgsStr = protoProxy.invoke('getUserMessagesExV2', userId, conversationTypes, lines, contentTypes, fromIndex, before, count, (protoMsgsStr) => {
            let protoMsgs = JSON.parse(protoMsgsStr);
            let msgs = [];
            protoMsgs.map(m => {
                let msg = Message.fromProtoMessage(m);
    ...
    async sendMessageEx(message, toUsers = [], preparedCB, progressCB, successCB, failCB) {
        ...
        if (message.messageContent instanceof MediaMessageContent && (message.messageContent.localPath || message.messageContent.file) && !message.messageContent.remotePath && this.isSupportBigFilesUpload()) {
            ...
            if ((process.platform === 'darwin' || process.platform === "win32") && message.messageContent instanceof ImageMessageContent) {
                ...
                if (message.messageContent.localPath) {
                    let thumbnail = await nativeImage.createThumbnailFromPath(message.messageContent.localPath, {width: 200, height: 200})
        ...
        if (message.messageContent instanceof MediaMessageContent) {
            if (!message.messageContent.localPath && message.messageContent.file) {
                ...
                let filePath = tmp.tmpNameSync() + file.name;
